...
do
  ...
  then
    ...
    cmd="srun python $executable_path/caribbean_model.py $c_start_date:0 $c_end_date:0 True True False $basename_$start_date_str_$fi"
  else
    # start from restart file
    cmd="srun python $executable_path/caribbean_model.py $c_start_date:0 $c_end_date:0 True True False $basename_$start_date_str_$fi $output_path/$basename_$start_date_str_$fp.nc"
  fi
  echo $cmd
  $cmd
